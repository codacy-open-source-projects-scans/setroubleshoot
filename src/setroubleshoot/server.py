...
"""
    ...
    def avc(self, data):
        ...
        try:
            ...
            for (record_type, event_id, body_text, fields, line_number) in self.record_reader.feed(data):
...
def RunFaultServer(timeout=10):
    ...
    try:
        ...
        for i in host_database.sigs.signature_list:
            ...
            if why in (audit2why.ALLOW, audit2why.DONTAUDIT, audit2why.BADSCON, audit2why.BADTCON, audit2why.BADTCLASS, audit2why.BADPERM):
                if why == audit2why.ALLOW:
                    reason = "it is allowed"
                elif why == audit2why.DONTAUDIT:
                    reason = "it is dontaudit'd"
                elif why == audit2why.BADSCON:
                    reason = "source context %s is undefined" % i.sig.scontext
                elif why == audit2why.BADTCON:
                    reason = "target context %s is undefined" % i.sig.tcontext
                elif why == audit2why.BADTCLASS:
                    reason = reason = "security class %s is undefined" % i.sig.tclass
                else:
                    reason = "permission %s is undefined" % ",".join(i.sig.access)
                syslog.syslog(syslog.LOG_NOTICE, "Deleting alert %s, %s in current policy" % (i.local_id, reason))
                deleted = True
